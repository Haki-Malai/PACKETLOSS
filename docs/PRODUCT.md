...
Pausing freezes movement and gameplay timers and places a dark scrim over the full-color scene. Menu backdrops do not resume play. Enter activates a menu's highlighted primary action when the panel itself has focus. Focused buttons retain their native Enter/Space behavior, and typing in the nickname field never triggers gameplay shortcuts. Menus contain keyboard focus and restore it on return, and gameplay/debug input is inactive while menus own control. Held keys and touch gestures are cleared across transitions.
