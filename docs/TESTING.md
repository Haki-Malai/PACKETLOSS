...
- Check score-bonus thresholds against real-collectible counts, fixed central placement, queued/missed pickups, a single combined award rounding, refresh without stacking, pause/death/level resets, tutorial exclusion, and no effect on maze-clear counts. Verify all five GLBs load, menu previews release resources, and the HUD reports whole-second status.
