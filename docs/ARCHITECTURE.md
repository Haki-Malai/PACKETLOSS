...
`WorldState.levelMultiplier` is the shared gameplay/scoring factor. The runtime passes scaled delta time to its scheduler and update systems while counting unscaled active play time. Movement systems also apply the multiplier to their per-tick distance because movement is center-clamped rather than delta-based. Score sources round their base award multiplied by the current factor. UI/menu timing and render cadence are not scaled.
