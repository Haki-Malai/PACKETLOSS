...
`ScoreBonusSystem` runs after ordinary collection and reads only the initial and remaining real-collectible counts. At fixed 35% and 70% thresholds it presents one of the five level-tier icons on a stable packet-reachable central tile, queuing a second icon if needed. Its pickup and boost timers use scaled simulation time; death removes the active factor and a clear checkpoint removes all bonus state. Bonus icons are excluded from the clear count and tutorial composition. Both award sources use one helper that rounds base × level × active bonus once. The HUD receives only changes to the visible icon/boost and whole-second countdown.
